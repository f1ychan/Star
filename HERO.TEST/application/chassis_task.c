...
//    }
		...
		if(robot_state.chassis_power_limit == 55)
		{
				chassis_move_control_loop->power_control.k = 450;
		...
		else if(robot_state.chassis_power_limit == 65)
		{
				chassis_move_control_loop->power_control.k = 520;
				chassis_move_control_loop->power_control.power_limit = 65;
				input_power = 6500;
		}
		else if(robot_state.chassis_power_limit == 70)
		{
				chassis_move_control_loop->power_control.k = 520;
				chassis_move_control_loop->power_control.power_limit = 70;
				input_power = 7000;
		}
		else if(robot_state.chassis_power_limit == 80)
		{
				chassis_move_control_loop->power_control.k = 550;
				chassis_move_control_loop->power_control.power_limit = 80;
				input_power = 8000;
		}
		else if(robot_state.chassis_power_limit == 90)
		{
				chassis_move_control_loop->power_control.k = 570;
				chassis_move_control_loop->power_control.power_limit = 90;
			  input_power = 9000;
		}
		else if(robot_state.chassis_power_limit == 110||robot_state.chassis_power_limit >120)
		{
...
if(get_cap.capvot>1800.0f){
		...
		else if(power_heat_data_t.chassis_power_buffer > 8.0&&power_heat_data_t.chassis_power_buffer <= 20.0)
		{
				input_power *= 1.0;
			chassis_move_control_loop->power_control.k = 470;
		}
		else if(power_heat_data_t.chassis_power_buffer < 8.0){
			input_power-=500.f;
		...
		if(chassis_move_control_loop->chassis_RC->mouse.press_r)
				...
				release_energy = 0;
		//c键释放功率
		if(release_energy && (get_cap.capvot > 1500.0f) && power_heat_data_t.chassis_power_buffer>10.0f)
		{
				chassis_move_control_loop->power_control.k += 300;
		}
