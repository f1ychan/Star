...
static void gimbal_init(gimbal_control_t *init)
{
	...
	const static fp32 pitch_vision_pid[3] = {PITCH_VISION_PID_KP, PITCH_VISION_PID_KI, PITCH_VISION_PID_KD};
		//继电器控制云台电源
		//HAL_GPIO_Write6tPin(GPIOE, GPIO_PIN_13, GPIO_PIN_RESET);
    //电机数据指针获取  
    ...
    //初始化电机pi
		...
		init->gimbal_pitch_motor.offset_ecd=2000;
		init->gimbal_yaw_motor.offset_ecd = 5180;
    ...
    init->gimbal_yaw_motor.min_relative_angle = -2.60f;  
		init->gimbal_pitch_motor.min_relative_angle = -0.05f;
    init->gimbal_pitch_motor.max_relative_angle = 1.00f;     
		angle_radto = 0;
		angle_sin =0;
		angle_cos =0;
}
...
//		static int8_t ctal;
//		if(control_loop->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_CTRL)
//		 ctal = 1;
//	 else
//		 ctal = 0;
//		if(robot_state.remain_HP<15 && ctal == 0)
//			control_loop->gimbal_pitch_motor.absolute_angle_set = -0.35f;
		//继电器控制云台电源
